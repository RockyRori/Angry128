...
curr_score = 0
# 加载并缩放图片
scaled_image = pygame.transform.scale(pygame.image.load('images/block.png'), (block_size, block_size))
back_image = pygame.transform.scale(pygame.image.load('images/back.png'), (block_size, block_size))
images = []
for x in [2, 4, 8, 16, 32, 64]:
    image = pygame.transform.scale(pygame.image.load('images/blocknum/%d.png' % x), (block_size, block_size))
    images.append(image)
back_ground = pygame.transform.scale(pygame.image.load('images/background.webp'), (screen_width // 2, screen_height))
...
for i in range(0, SoundIdx.MAX):
    sound = pygame.mixer.Sound('sounds/%d.MP3' % i)
    SOUNDS.append(sound)
